...
**Current capability status on `main`.**
...
| **CFD / wind** | `cfd_laminar_channel_2d_v1` and `cfd_openfoam_laminar_internal_3d_v1`, `implementation_status=REAL` | Fully developed 2D plane-Poiseuille flow; steady 3D incompressible Newtonian laminar internal flow | Direct finite difference (2D); OpenFOAM Foundation 14 / `20260724` finite volume (3D) | Rectangular 2D fields; certified CAD-derived hex-dominant/polyhedral FV meshes | Analytical Poiseuille validation; server-owned square-duct analytical benchmark and real three-mesh convergence | 2D analytically validated; 3D `PARTIALLY_VALIDATED`, maximum trust `MODERATE` | No turbulence, transient, compressible, multiphase, external-aerodynamic, moving-mesh, CHT, FSI, or general industrial-accuracy claim | General CFD outside the declared bounded regimes | Additional benchmark families are required before broader trust claims |
