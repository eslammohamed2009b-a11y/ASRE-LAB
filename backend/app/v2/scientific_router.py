...
class BenchmarkRequest(BaseModel): inputs:dict[str,float];computed_result:float;source_simulation_id:str
...
class TrustRequest(BaseModel):
    ...
    computed_result:float;source_simulation_id:str;convergence_values:list[float]|None=None
...
def execute_benchmark(solver_id:str,payload:BenchmarkRequest,user:dict=Depends(get_current_user)):
    try:return benchmark(_item(solver_id),payload.inputs,payload.computed_result,payload.source_simulation_id)
...
def create_trust(payload:TrustRequest,user:dict=Depends(get_current_user)):
    ...
    try:bench=benchmark(item,payload.benchmark_inputs,payload.computed_result,payload.source_simulation_id)
    except ValueError as exc:raise HTTPException(422,str(exc))
