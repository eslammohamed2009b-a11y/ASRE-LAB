...
from app.module2_simulation.coupling import CouplingNotFoundError, ThermalStructuralCouplingRequest, ThermalStructuralCouplingResponse, get_coupling, run_thermal_structural_coupling

router = APIRouter(prefix="/api/couplings", tags=["Module 2 - Coupled Analysis"], dependencies=[Depends(get_current_user)])

@router.post("/thermal-structural", response_model=ThermalStructuralCouplingResponse)
...
@router.get("/{coupling_id}", response_model=ThermalStructuralCouplingResponse)
