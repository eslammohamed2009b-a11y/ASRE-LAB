...
SOLVER_REGISTRY.update({
    ...
    "cfd_openfoam_laminar_internal_3d_v1": CapabilityEntry(
        ...
        discretization="Authoritative CAD BRep-derived certified OpenFOAM blockMesh + snappyHexMesh finite-volume polyMesh",
        supported_dimensions=["3d"], supported_geometry=["authoritative_cad_fluid_volume_certified_fv"],
        ...
        consumes_cad_geometry=True, consumes_authoritative_cad=True, required_mesh_dimension=3,
        accepted_element_types=["hex8", "polyhedron"], supported_domain_types=["fluid"],
        geometry_dependency_description="Consumes a certified CFDGeneratedMeshV1 derived server-side from the same authoritative CAD BRep and semantic topology.",
        ...
        required_inputs=["matching CFD PhysicsModelV1", "explicit FLUID CAD volume", "certified CAD-derived finite-volume mesh"],
    ...
    "acoustic_helmholtz_fem_3d_v1": CapabilityEntry(
        solver_id="acoustic_helmholtz_fem_3d_v1", family=SolverFamily.WAVE_ACOUSTIC, version="1.0.0",
        implementation_status=ImplementationStatus.REAL, validation_status=ValidationStatus.PARTIALLY_VALIDATED,
        governing_equations=["Frequency-domain Helmholtz equation: Laplacian(P) + k^2 P = 0", "Time convention: p(x,t) = Re{P exp(i omega t)}"],
        numerical_method="Complex sparse Galerkin solve with scipy.sparse on first-order TET4 elements",
        discretization="Authoritative CAD-derived SI TET4 mesh; exact constant-gradient stiffness and consistent tetrahedral mass integration",
        supported_dimensions=["3d"], supported_geometry=["authoritative_cad_acoustic_tetra4"],
        geometry_limitations="One fixed homogeneous isotropic acoustic volume with no mean flow; explicit acoustic domain only.",
        consumes_cad_geometry=True, consumes_authoritative_cad=True, required_mesh_dimension=3,
        accepted_element_types=["tetra4"], supported_domain_types=["acoustic"],
        geometry_dependency_description="Consumes GeneratedMesh nodes, TET4 connectivity, semantic surface mappings, and PhysicsModelV1 identities directly.",
        supported_materials=["materials with density and speed_of_sound"],
        supported_boundary_conditions=["acoustic_pressure", "acoustic_wall:rigid", "acoustic_wall:pressure_release", "acoustic_wall:impedance"],
        required_inputs=["PhysicsModelV1", "matching authoritative CAD-derived TET4 mesh", "frequency_hz", "density", "speed_of_sound"],
        output_metrics=["complex nodal pressure", "pressure amplitude", "pressure phase", "SPL", "algebraic residual", "k*h_max", "reciprocal condition estimate"],
        validity_envelope={"nodes": "<= 5000", "elements": "<= 20000", "dispersion": "k*h_max <= 0.5", "residual": "<= 1e-8", "conditioning": "sparse one-norm reciprocal condition estimate >= 1e-10"},
        convergence_requirements="Finite complex field, normalized free-DOF algebraic residual <= 1e-8, k*h_max <= 0.5, and sparse reciprocal condition estimate >= 1e-10 are required.",
        implementation_reference="app.module2_simulation.acoustic_fem_solvers.solve_acoustic_fem_3d",
        known_limitations=["Lossless stationary homogeneous isotropic fluid with no mean flow.", "No radiation, PML, infinite elements, coupling, transient, thermoviscous, porous, nonlinear, or volumetric source support.", "Validation is limited to the server-owned rectangular plane-wave gate; no general acoustics claim."],
        benchmark_references=["tests/integration/test_acoustic_fem_3d.py::test_real_acoustic_3d_plane_wave_accuracy", "tests/integration/test_acoustic_fem_3d.py::test_real_acoustic_3d_plane_wave_refinement"],
    ),
})
...
_CONTRACT_DETAILS: dict[str, dict] = {
    ...
    "acoustic_helmholtz_fem_3d_v1": {"numerical_method": "Complex sparse Galerkin solve with scipy.sparse on first-order TET4 elements", "discretization": "Authoritative CAD-derived SI TET4 mesh with exact tetrahedral stiffness and mass integration", "supported_geometry": ["authoritative_cad_acoustic_tetra4"], "consumes_cad_geometry": True, "geometry_dependency_description": "Consumes authoritative CAD-derived mesh and PhysicsModelV1; no scalar geometry reconstruction.", "validity_envelope": {"nodes": "<= 5000", "elements": "<= 20000", "dispersion": "k*h_max <= 0.5", "residual": "<= 1e-8", "conditioning": "sparse one-norm reciprocal condition estimate >= 1e-10"}, "convergence_requirements": "Finite complex pressure, normalized free-DOF algebraic residual <= 1e-8, k*h_max <= 0.5, and sparse reciprocal condition estimate >= 1e-10 are required.", "implementation_reference": "app.module2_simulation.acoustic_fem_solvers.solve_acoustic_fem_3d"},
