...
_CONTRACT_DETAILS: dict[str, dict] = {
    ...
    "thermal_conduction_v1": {"numerical_method": "Finite-difference iterative solution", "discretization": "1D uniform 2-500-node rod/slab or 3D uniform 5-40-node cubic grid", "supported_geometry": ["uniform_1d_rod_or_slab", "uniform_cubic_3d_domain"], "consumes_cad_geometry": False, "geometry_dependency_description": "Uses requested scalar dimensions and uniform-grid settings only; does not consume design artifacts or CAD meshes.", "validity_envelope": {"1d_nodes": "2 to 500", "3d_nodes_per_edge": "5 to 40", "geometry": "uniform rod/slab or cube only"}, "convergence_requirements": "Use refinement where numerical resolution affects the requested result; no automatic convergence study is supplied.", "implementation_reference": "app.module2_simulation.solvers.thermal_solver.ThermalConductionSolver"},
    ...
    "cfd_laminar_channel_2d_v1": {"numerical_method": "Analytical plane-Poiseuille velocity profile sampled on a regular grid", "discretization": "5-60 node rectangular sampling grid", "supported_geometry": ["infinite_parallel_plate_channel"], "consumes_cad_geometry": False, "geometry_dependency_description": "Uses requested channel height/length and regular-grid settings; no CAD or obstacle geometry is consumed.", "validity_envelope": {"grid_size": "5 to 60", "flow": "steady fully developed laminar Re < 2000"}, "convergence_requirements": "Grid refinement may be reported for sampled output; the governing velocity profile is analytical within the stated model.", "implementation_reference": "app.module2_simulation.solvers.channel_flow_solver.LaminarChannelFlowSolver"},
    "acoustic_duct_1d_v1": {"numerical_method": "Analytical 1D Helmholtz pressure solution sampled along the duct", "discretization": "4-500 axial samples/elements", "supported_geometry": ["uniform_straight_duct"], "consumes_cad_geometry": False, "geometry_dependency_description": "Uses scalar duct length and axial sampling; no CAD geometry is consumed.", "validity_envelope": {"num_elements": "4 to 500", "geometry": "uniform straight lossless 1D duct"}, "convergence_requirements": "Increase axial samples when output sampling resolution matters; the bounded 1D solution is analytical.", "implementation_reference": "app.module2_simulation.solvers.acoustic_solver.AcousticDuctSolver"},
